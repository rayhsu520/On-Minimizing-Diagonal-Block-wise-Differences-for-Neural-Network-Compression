...
parser.add_argument('--train-mode', '-tm',type=int, default=1, metavar='P',
                    help='1: initial training 2: pre training 3:quantizatize training 4:initial+pruning+quantize 5: load model+pruning+quantize')
...
# ------------ save folder ---------------------------------------------------------------------------
parser.add_argument('--save-dir', '-sd', default='model_1', type=str,                                    
...
train_loader = torch.utils.data.DataLoader(
    ...
    batch_size=args.batch_size, shuffle=True,pin_memory=True)# no idea why I can't use data loader
...
def main():
    ...
    if args.train_mode == 1:
        model = LeNet5( mask=True).to(device)
...
def pruning_process(model):
    ...
    if args.model_mode == 'd' or args.model_mode == 'a':
        model.prune_by_percentile( ['fc1'], q=100-8.0)
