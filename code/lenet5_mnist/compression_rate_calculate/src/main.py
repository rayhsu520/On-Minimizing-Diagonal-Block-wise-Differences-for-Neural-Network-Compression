...
parser = argparse.ArgumentParser(description='PyTorch huffman coding')
...
#--------------------different method----------------------------------------
    ...
    fc_compressed_without_edit_mpd_alpha0 = fc_compressed_without_edit_mpd
...
#----------- plot fc space on different alpha with editgraph------------------ 
    ...
    for a,b in zip(y_pos,acc_alpha):
        plt.text(a-0.25, b-4, "{0:.2f}".format(b), fontsize=10)
    ax2.set_ylim(0, 100)
    ax2.set_xlabel('Alpha', fontsize=14)
    ax2.set_ylabel('Accuracy (%)', fontsize=14)
    ax1.plot(np.nan, '-ro', label = 'Accuracy')
    ax1.legend(loc='lower right')
    plt.savefig(f'{args.save_dir}/lenet5_space_penalty_less.png')
    plt.close()
#----------- plot comparison of different compression methods ------------------ 
    ...
    plt.title('Compression results on LeNet5')
