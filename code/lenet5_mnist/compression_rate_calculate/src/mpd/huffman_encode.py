...
import mpd.LeNet5_mask as LeNet5_mask
...
def mpd_huffman_encode(val_loader ,model_path ,args):
    ...
    model = LeNet5_mask.LeNet5_mask('LeNet5_mask', args.partition, mask_flag=True).cuda()
    model = util.load_checkpoint_withpermu(model,  f"{model_path}",args)
    ...
    for name, param in model.named_parameters():
        ...
        if 'conv' in name and args.train_mode is not 'd':
            ...
            print(fc_edit.shape)
            print(fc_first_block.shape)

            fc_bit, fc_compressed_bit=huffman_encode_model(fc_first_block.astype(numpy.int32))
            org_total += fc_bit
            ...
            if partition is not 1:
                fc_edit_bit, fc_edit_compress=huffman_encode_model(fc_edit.astype(numpy.float32))
...
#----------- compress with editgraph format ------------------------------------------
            ...
            print('---------first block-----------')
            print(fc_first_block)
            fc_bit, fc_compressed_bit, t0, d0=huffman_encode_model(fc_first_block.astype(numpy.float32))
            org_total += fc_bit
            compressed += fc_compressed_bit
            fc_org_total += fc_bit
            ...
            util.log(f"{args.log_detail}", f"\tedit original:{fc_edit_bit} bytes\t edit block after:{fc_edit_compress} bytes")
            util.log(f"{args.log_detail}", f"original:{fc_bit+fc_edit_bit} bytes\t after:{fc_compressed_bit+fc_edit_compress} bytes")
            layer_compressed_dic[name[0:3]] = fc_compressed_bit + fc_edit_compress
