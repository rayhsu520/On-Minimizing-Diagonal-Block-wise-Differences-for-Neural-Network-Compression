...
def initial_train(model, args, train_loader, val_loader, tok, use_cuda=True):
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        if(prec1 > best_prec1):
            ...
            model = save_masked_checkpoint(model, tok, is_best, best_prec1, epoch, args)
...
def quantized_retrain(model, args, quantized_index_list, quantized_center_list, train_loader, val_loader, use_cuda=True):
    ...
    for epoch in range(args.qauntize_epochs):
        ...
        if(prec1 > best_prec1):
            is_best = prec1 > best_prec1
            best_prec1 = max(prec1, best_prec1)
            model = save_masked_checkpoint(model, 'quantized_re', is_best, best_prec1, epoch, args)
    model = save_masked_checkpoint(model,'quantized_re', True, best_prec1, args.qauntize_epochs, args)
    return model
...
def validate(val_loader, model, args):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        # compute output
        output = model(input_var)
        loss = criterion(output, target_var)+args.alpha*fc_penalty(model,penalty)
        output = output.float()
        loss = loss.float()
        # measure accuracy and record loss
        prec1 = accuracy(output.data, target)[0]
...
def fc_penalty(model, penalty):
    ...
    #index slice weight[start:end:step(row),start:end:step:step(col)]
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec {top1.val:.3f} ({top1.avg:.3f})'.format(
                      epoch, i, len(train_loader), batch_time=batch_time,
                      data_time=data_time, loss=losses, top1=top1))
            print(alpha*fc_penalty(model,penalty))
