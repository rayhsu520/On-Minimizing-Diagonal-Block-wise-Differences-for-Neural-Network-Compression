...
def quantized_retrain(model, args, quantized_index_list, quantized_center_list, train_loader, use_cuda=True):
    ...
    model.train()
    for epoch in range(args.reepochs):
...
def initial_train(model, args, train_loader, val_loader, tok, use_cuda=True):
    ...
    for epoch in range(args.start_epoch, epochs):
        train(train_loader, model, criterion, optimizer, epoch, args, tok)
        ...
        if(prec1 > best_prec1):
            ...
            }, is_best, filename=os.path.join(args.save_dir, 'checkpoint_{}_{}.tar'.format(tok,epoch)))
    save_checkpoint({
       'epoch': epoch + 1,
       'state_dict': model.state_dict(),
       'best_prec1': best_prec1,
    }, True, filename=os.path.join(args.save_dir, 'checkpoint_{}_{}.tar'.format(tok,epochs)))
...
def accuracy(output, target, topk=(1,)):
    ...
    for k in topk:
        ...
        res.append(correct_k.mul_(100.0 / batch_size))
    return res
def train(train_loader, model, criterion, optimizer, epoch, args, tok):
    """
        Run one train epoch
    """
    batch_time = AverageMeter()
