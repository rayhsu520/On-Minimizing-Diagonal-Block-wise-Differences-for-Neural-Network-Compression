...
#--------------------different method----------------------------------------
    ...
    acc_alpha.append(acc_mpd[0].item())
...
#----------- plot fc space on different alpha with editgraph------------------ 
    ...
    lns1 = ax1.bar(y_pos, compressed_rate, align='center',color='Blue', label='Compression Rate')
    ...
    ax2 = ax1.twinx()
    lns2 = ax2.plot(y_pos, acc_alpha, '-ro', label='Accuracy')
    ...
    plt.savefig(f'{args.save_dir}/vgg16-10_space_penalty.png')
    plt.close()
    print('alpha compression bytes', compressed_alpha)
#----------- plot comparison of different compression methods ------------------ 
    ...
    plt.yscale('log')
    plt.xlabel('Compress algorithm', fontsize=14)
    plt.ylabel('Compression rate', fontsize=14)
    plt.savefig(f'{args.save_dir}/compress_result.eps',format='eps')
