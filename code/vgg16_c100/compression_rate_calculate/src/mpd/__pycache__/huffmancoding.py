...
def huffman_encode_editgraph(model, maxdiff,directory='encodings/'):
    original_total = 0
    compressed_total = 0
    ...
    t1, d1 = huffman_encode(calc_indice_diff(mat.indices, maxdiff), directory)
    t2, d2 = huffman_encode(calc_index_diff(mat.indptr, maxdiff), directory)
...
'''def huffman_decode_model(model, directory='encodings/'):
    ...
    data = huffman_decode(directory, name+f'_{form}_data', dtype='float32')
            ...
            # Construct matrix #have to work on this
    mat = matrix(data, shape)
