...
#--------------------different method----------------------------------------
    ...
    acc_alpha.append(float(acc_mpd.item()))
    print(float(acc_mpd.item()))
    ...
    while alpha <=0.1:
        ...
        alpha*=10
        split_list[4] = str(alpha)
        if float(acc_mpd.item()) > best_alpha_acc:
            best_alpha_acc = float(acc_mpd.item())
            best_alpha = alpha
...
#----------- plot fc space on different alpha without editgraph---------
    compressed_rate=[]
    ...
    for a,b in zip(y_pos, compressed_rate):
        ax1.text(a-0.25, b+0.1, str(b)+'x',fontsize=10)
    ...
    for a,b in zip(y_pos,acc_alpha):
        ax2.text(a-0.25, b+2, "{0:.2f}".format(b))
    ax2.set_ylim(0, 100)
    ax2.set_xlabel('Alpha',fontsize=14)
    ax2.set_ylabel('Accuracy (%)',fontsize=14)
    ax1.plot(np.nan, '-ro', label = 'Accuracy')
    ax1.legend(loc='lower right')
    plt.savefig(f'{args.save_dir}/fc_space_penalty.png')
    plt.close()
#----------- plot comparison of different compression methods ------------------ 
    ...
    acc_list = [acc_init.item(), acc_onlyq.item(), acc_deep.item(), float(acc_mpdp.item()), acc_alpha[0], best_alpha_acc]
    ...
    plt.title('Compression results on VGG16 (Cifar100)')
