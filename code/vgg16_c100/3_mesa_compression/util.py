...
def initial_train(model, args, train_loader, val_loader, tok, use_cuda=True):
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        if(prec1 > best_prec1):
            ...
            save_masked_checkpoint(model, tok, is_best, best_prec1, epoch, args)
    save_masked_checkpoint(model, tok, True, best_prec1, epochs, args)
    return model
def quantized_retrain(model, args, quantized_index_list, quantized_center_list, train_loader, val_loader, use_cuda=True):
    ...
    for epoch in range(args.qauntize_epochs):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            #    print(args.alpha*fc_penalty(model,penalty))
        # evaluate on validation set
        prec1 = validate(val_loader, model, args)
        # remember best prec1 and save checkpoint
        if(prec1 > best_prec1):
            is_best = prec1 > best_prec1
            best_prec1 = max(prec1, best_prec1)
            save_masked_checkpoint(model, 'quantized_re', is_best, best_prec1, epoch, args)
    save_masked_checkpoint(model,'quantized_re', True, best_prec1, args.qauntize_epochs, args)
    return model
