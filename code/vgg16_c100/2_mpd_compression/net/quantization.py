...
def sequential_weight_sharing(model, model_mode, bits=4):
    ...
    for name, module in model.named_children():
        ...
        # dense layer
            ...
            kmeans.fit(mat.reshape(-1,1))
            cluster_weight = kmeans.cluster_centers_[kmeans.labels_].reshape(-1)
            cluster_weight = cluster_weight.reshape(blocks.shape)

            t = tuple(cluster_weight)
            ...
            quantized_center = kmeans.cluster_centers_
            if 'fc1' in name:
                temp=torch.index_select(torch.from_numpy(quantized_index), 0, model.rowp1)
                quantized_index = torch.index_select(temp, 1, model.colp1).numpy()
            if 'fc2' in name:
                temp=torch.index_select(torch.from_numpy(quantized_index), 0, model.rowp2)
                quantized_index = torch.index_select(temp, 1, model.colp2).numpy()
            if 'fc3' in name:
                temp=torch.index_select(torch.from_numpy(quantized_index), 0, model.rowp3)
                quantized_index = torch.index_select(temp, 1, model.colp3).numpy()
