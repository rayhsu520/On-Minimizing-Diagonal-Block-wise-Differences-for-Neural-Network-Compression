...
def main():
    ...
    if args.train_mode == 1:
        ...
        model = initial_process(model)
        model = pruning_process(model)
        print('time consume:init+prune',time.time()-st)
        quantize_process(model)
        print('time consume:quantize',time.time()-st)
...
def quantize_process(model):
    ...
    util.quantized_retrain(model, args, quantized_index_list, quantized_center_list, train_loader, test_loader, use_cuda)
    acc = util.validate(args, test_loader, model, criterion)
